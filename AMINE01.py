...
def compute_scores(age, tabac, fragments, HSA, gap, bone_quality, comorbidities):
    """
    Returns S_AVN, S_PSEU, S_FAIL_FIX, S_SURG
    Calibrated based on PHARON/meta-analyses.
    """
...
def propose_treatment(S_AVN, S_PSEU, S_FAIL_FIX, age, fragments, gap, bone_quality, comorbidities):
    """
    Returns (treatment_label, justification)
    Options:
        - Conservative (Non-operative)
        - Closed reduction & internal fixation (IM nailing)
        - Open reduction & internal fixation (ORIF)
        - Arthroplasty (HA / RTSA)
    """
    ...
    if S_AVN >= 40 or f >= 3:
        if age >= 75 or bone_quality == "poor":
            return (tr("Arthroplasty (per-op type)","Arthroplastie (type selon per-op)"), tr("High-risk signals; arthroplasty preferred for patient profile.","Signal de risque élevé; arthroplastie privilégiée en raison du profil du patient."))
        else:
            return (tr("Open reduction internal fixation (ORIF)","Ostéosynthèse foyer ouvert (ORIF)"), tr("Moderate-high risk signals; patient young/good bone → attempt reconstruction.","Signal de risque modéré-élevé mais patient jeune/osseux correct -> tenter reconstruction."))

    return (tr("Conservative treatment","Traitement orthopédique (conservateur)"), tr("Default conservative choice.","Choix conservateur par défaut."))
...
def page_research():
    ...
    st.markdown(tr("**Archiving advice**: version on GitHub, Zenodo DOI, save PDFs in `research/` folder, backup DB regularly.",
                   "**Conseils d'archivage** : versionner le dépôt (GitHub), archiver sur Zenodo (DOI), stocker PDFs dans `research/` et sauvegarder la DB régulièrement."))
