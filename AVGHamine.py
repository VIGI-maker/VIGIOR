...
def page_registered_patients():
    ...
    st.markdown("**Age distribution / Distribution des âges**")
    ...
    st.markdown("**Average HSA angle / Moyenne angle HSA**")
    st.write(df["HSA"].mean())

    st.markdown("**Osteoporosis (Bone quality = poor) / Ostéoporose**")
    st.write(f"{(df['BoneQuality']=='poor').mean()*100:.1f}%")

    st.markdown("**Nonunion / Pseudarthrose (S_PSEU >= 30)**")
    st.write(f"{(df['S_PSEU']>=30).mean()*100:.1f}%")

    st.markdown("**Avascular necrosis / Nécrose tête humérale (S_AVN >= 30)**")
    st.write(f"{(df['S_AVN']>=30).mean()*100:.1f}%")

    st.markdown("**Fixation failure / Échec de fixation (S_FAIL_FIX >= 30)**")
    st.write(f"{(df['S_FAIL_FIX']>=30).mean()*100:.1f}%")
